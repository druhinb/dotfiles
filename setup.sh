...
install_apt_packages() {
  ...
  sudo_run apt-get install -y \
    zsh tmux vim fzf ripgrep fd-find bat eza zoxide thefuck \
...
install_rust_tools() {
  ...
  if have starship && have atuin && have yazi && have ya; then
    return
  fi
...
link_dotfiles() {
  ...
  link_file "$DOTFILES_DIR/tmux/.tmux" "$HOME/.tmux"
...
install_zsh_plugins() {
  ...
  clone_if_missing https://github.com/MichaelAquilina/zsh-you-should-use "$plugin_dir/you-should-use"
}
...
install_tmux_plugins() {
  ...
  if [[ -d "$DOTFILES_DIR/.git" ]]; then
    run git -C "$DOTFILES_DIR" submodule update --init --recursive tmux/.tmux/plugins || true
  fi

  clone_if_missing https://github.com/tmux-plugins/tpm "$HOME/.tmux/plugins/tpm"

  # TPM treats empty directories as installed plugins. Remove empty placeholders
  # so it can clone the plugin contents when submodules were not initialized.
  if [[ "$DRY_RUN" == 0 && -d "$HOME/.tmux/plugins" ]]; then
    find "$HOME/.tmux/plugins" -mindepth 1 -maxdepth 1 -type d -exec rmdir {} + 2>/dev/null || true
  fi
...
install_neovim_plugins() {
  ...
  run nvim --headless \
    "+MasonInstall stylua clang-format cpplint clangd ruff prettier eslint_d prettierd shellcheck shfmt gofumpt goimports golangci-lint markdownlint yamllint sqlfluff google-java-format basedpyright typescript-language-server svelte-language-server html-lsp css-lsp emmet-language-server rust-analyzer asm-lsp gopls marksman ltex-ls lua-language-server json-lsp yaml-language-server taplo bash-language-server dockerfile-language-server docker-compose-language-service sqlls codelldb delve debugpy js-debug-adapter" \
