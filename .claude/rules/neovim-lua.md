...
- fzf-lua is the preferred picker whenever the `fzf` binary is present, including over SSH. telescope.nvim (`custom/plugins/telescope.lua`) is the fallback when `fzf` isn't installed; preserve the lightweight built-in fallbacks in `lua/search.lua` for hosts with neither.
